...
function getApiKey(): string {
  ...
  console.error("成功获取到 BAIDU_MAP_API_KEY");
  return apiKey;
}
...
async function handleWeather(districtId?: string, location?: string) {
...
async function handlePoiExtract(textContent: string) {
  ...
  if (submitData.status !== 0) {
    return {
      content: [
        {
          type: "text",
          text: `mark submit failed: ${
            submitData.message || submitData.status
          }`,
        },
      ],
      isError: true,
  ...
  try {
    switch (request.params.name) {
      ...
      case "map_search_places": {
        const { query, region, bounds, location } = request.params
          .arguments as {
          query: string;
          region?: string;
          bounds?: string;
      ...
      case "map_road_traffic": {
        const { roadName, city, bounds, vertexes, center, radius } = request
          .params.arguments as {
          roadName?: string;
          city?: string;
          bounds?: string;
        ...
        return await handleRoadTraffic(
          roadName,
          city,
          bounds,
          vertexes,
